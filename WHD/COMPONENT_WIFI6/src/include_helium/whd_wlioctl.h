...
/* MBO add Channel Preference */
typedef struct mbo_add_chan_pref {
    mbo_xtlv_t opclass;
    mbo_xtlv_t chan;
    mbo_xtlv_t pref;
    mbo_xtlv_t reason;
...
/* MBO delete Channel Preference */
typedef struct mbo_del_chan_pref {
    mbo_xtlv_t opclass;
    mbo_xtlv_t chan;
} mbo_del_chan_pref_t;
...
typedef struct {
        ...
        /* Template underflows (mac was too slow to transmit ACK/CTS or BCN) */
        uint32  txtplunfl;
...
{
    uint32_t lostfrmcnt;              /* RX PER test: no of frames lost (skip seqno) */
    int rssi;                     /* RSSI */
    int snr;                      /* signal to noise ratio */
    uint16_t rxpktcnt[NUM_80211_RATES+1];
    uint8_t rssi_qdb;                 /* qdB portion of the computed rssi */
    uint8_t  version;
    uint16_t length;
    uint16_t pad;
    int rssi_per_core[WL_RSSI_ANT_MAX];
    int rssi_per_core_qdb[WL_RSSI_ANT_MAX];
} wl_pkteng_stats_t;
...
typedef struct wlc_csi_cfg {
	...
	int32_t capture_period_ms;	/* -1: Disable periodic CSI capture */
	uint16_t capture_window_dur_ms;
	uint8_t solicit_mode;			/* 0: Unsolicited Mode 1: Solicited Mode */
	uint8_t assoc_mode;			/* 0: Unassociated Mode 1: Associated Mode */

	uint8_t bss_mode; 			/* 0: Allow all Rx   1: My BSS  2: Other BSS */
	...
	uint8_t multi_csi_per_mac;		/* Capture multiple CSI per mac address */

	/* Associated Mode only */
	uint8_t link_protection;		/* Enable PM indication before CSI window */

	/* Not Implemented */
	uint8_t subcarriers;			/* Capture CSI  only from selected subcarriers */
