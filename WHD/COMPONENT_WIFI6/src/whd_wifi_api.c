...
#define WIFI_JOIN_MIN_TIMEOUT            (2000)   /* Minimum WiFi join timeout value */
#define WIFI_JOIN_MAX_TIMEOUT            (10000)  /* Maximum WiFi join timeout value */
...
#define EVTLOG_LOG_AND_PRINT_MODE       0xC0
...
const whd_event_num_t join_events[]  =
{
    ...
    WLC_E_REASSOC, WLC_E_PROBRESP_MSG, WLC_E_NONE
};
...
static const whd_event_num_t twt_setup_events[] =
{ WLC_E_TWT_SETUP, WLC_E_TWT_TEARDOWN, WLC_E_NONE };
...
typedef struct {
    ...
    uint8_t set;
    uint8_t flags;
}wl_el_tag_params_s_t;

typedef struct {
    uint8_t start_idx;
    uint8_t iterations;
}start_ptr_and_iter_t;
...
static whd_result_t whd_wifi_active_join_init(whd_interface_t ifp, whd_security_t auth_type,
                                          ...
                                          cy_semaphore_t *semaphore, whd_bool_t is_reassoc);
...
whd_result_t whd_wifi_set_up(whd_interface_t ifp)
{
    ...
    if (whd_wifi_get_chip_serial(ifp, version, sizeof(version)) == WHD_SUCCESS)
    {
        WPRINT_MACRO(("chip %s", version));
    }
    whd_mem_memset(version, 0, sizeof(version));
    ...
    {
        WPRINT_WHD_VERSION( ("WLAN Firmware    : %s, Chip ID : %d", version, whd_chip_get_chip_id(whd_driver)) );
    }
    ...
    WPRINT_WHD_VERSION( ("WHD VERSION      : " WHD_VERSION) );
...
whd_result_t whd_get_bt_info(whd_driver_t whd_driver, whd_bt_info_t bt_info)
{
    ...
    whd_mem_memset(bt_info, 0, sizeof(struct whd_bt_info) );
...
{
    uint32_t band_list[4] = { 0 };
    whd_band_list_t whd_bandlist = { 0 };
    uint32_t res = 0,i = 0,j = 0;
    char band_str[10] = { 0 };

    res = whd_wifi_get_ioctl_buffer(ifp, WLC_GET_BANDLIST, (uint8_t*)&band_list, sizeof(band_list));
    if(res != WHD_SUCCESS)
    {
        WPRINT_WHD_ERROR( ("Function whd_wifi_get_ioctl_buffer failed at line %d \n ", __LINE__) );
        return false;
    }
    /*
     ...
     */
    whd_bandlist.number_of_bands= band_list[0];
    WPRINT_WHD_DEBUG( ("Number of supported bands: %ld\n",whd_bandlist.number_of_bands ) );

    for (i = 1; i <= whd_bandlist.number_of_bands && i < ARRAY_SIZE(band_list); i++,j++)
    {
        switch (band_list[i])
        {
            case WLC_BAND_6G:
                strcpy(band_str,"6GHz");
                break;
            case WLC_BAND_5G:
                strcpy(band_str,"5GHz");
                break;
            case WLC_BAND_2G:
                strcpy(band_str,"2.4GHz");
                break;
            default:
                strcpy(band_str,"Unknown");
                break;
        }
        whd_bandlist.other_band[j] = band_list[i];
        WPRINT_WHD_DEBUG( ("%s band supported\n", band_str) );
        if (band_list[i] == WLC_BAND_6G)
        {
            return true;
        }
    }
...
#else
    whd_mem_memcpy(data, &bss_index, sizeof(uint32_t));
    whd_mem_memcpy(data + sizeof(bss_index), &eapol_key_timeout, sizeof(uint32_t) );
#endif
    CHECK_RETURN(whd_proto_set_iovar(ifp, buffer, 0) );
...
whd_result_t whd_wifi_set_passphrase(whd_interface_t ifp, const uint8_t *security_key, uint8_t key_length)
{
    ...
    whd_mem_memset(psk, 0, sizeof(wsec_pmk_t) );
...
whd_result_t whd_wifi_sae_password(whd_interface_t ifp, const uint8_t *security_key, uint8_t key_length)
{
    ...
    whd_mem_memset(sae_password, 0, sizeof(wsec_sae_password_t) );
    whd_mem_memcpy(sae_password->password, security_key, key_length);
...
whd_result_t whd_wifi_offload_config(whd_interface_t ifp, uint32_t ol_feat, uint32_t reset)
{
    ...
    whd_mem_memset(ol_cfg, 0, sizeof(wl_ol_cfg_v1_t) );

#ifdef PROTO_MSGBUF
    ol_cfg_cpy.ver = WL_OL_CFG_VER_1;
    ol_cfg_cpy.len = sizeof(wl_ol_cfg_v1_t);
    ol_cfg_cpy.id = WL_OL_CFG_ID_PROF;
    ol_cfg_cpy.offload_skip = ol_feat_skip;
    memcpy(ol_cfg , &ol_cfg_cpy, sizeof(wl_ol_cfg_v1_t) );
#else
    ...
    if(whd_configure_tko_offload(ifp, WHD_TRUE) != WHD_SUCCESS)
    {
        WPRINT_WHD_ERROR( ("Could not configure TKO or chip does not support it\n") );
    }
    else
    {
        WPRINT_WHD_INFO( ("TKO configured\n") );
    }
...
whd_result_t whd_wifi_offload_ipv4_update(whd_interface_t ifp, uint32_t ol_feat, uint32_t ipv4_addr, whd_bool_t is_add)
{
    ...
    ol_cfg_cpy.ver = WL_OL_CFG_VER_1;
    ol_cfg_cpy.len = sizeof(wl_ol_cfg_v1_t);
    ol_cfg_cpy.id = WL_OL_CFG_ID_INET_V4;
    ol_cfg_cpy.offload_skip = ol_feat_skip;
    memcpy(ol_cfg , &ol_cfg_cpy, sizeof(wl_ol_cfg_v1_t) );
#else
    ol_cfg->ver = WL_OL_CFG_VER_1;
    ...
    whd_mem_memcpy(&ol_cfg->u.ol_inet_v4.host_ipv4, &addr, sizeof(addr));
...
whd_result_t whd_wifi_offload_ipv6_update(whd_interface_t ifp, uint32_t ol_feat, uint32_t *ipv6_addr, uint8_t type, whd_bool_t is_add)
{
    ...
    ol_cfg_cpy.ver = WL_OL_CFG_VER_1;
    ol_cfg_cpy.len = sizeof(wl_ol_cfg_v1_t);
    ol_cfg_cpy.id = WL_OL_CFG_ID_INET_V6;
    ol_cfg_cpy.offload_skip = ol_feat_skip;
    memcpy(ol_cfg , &ol_cfg_cpy, sizeof(wl_ol_cfg_v1_t) );
#else
    ol_cfg->ver = WL_OL_CFG_VER_1;
    ...
    whd_mem_memcpy(&ol_cfg->u.ol_inet_v6.host_ipv6, &addr, sizeof(addr));
...
#ifdef PROTO_MSGBUF
    ol_cfg_cpy.len = sizeof(wl_ol_cfg_v1_t);
    ol_cfg_cpy.id = WL_OL_CFG_ID_ACTIVATE;
    ol_cfg_cpy.offload_skip = ol_feat_skip;
    memcpy(ol_cfg , &ol_cfg_cpy, sizeof(wl_ol_cfg_v1_t) );
#else
    ol_cfg->ver = WL_OL_CFG_VER_1;
...
whd_result_t whd_wifi_keepalive_config(whd_interface_t ifp, whd_keep_alive_t *packet, uint8_t flag )
{
	...
	if (flag == WHD_KEEPALIVE_NULL )
	{
		...
		whd_mem_memset(keepalive_cfg, 0, sizeof(wl_keep_alive_pkt_t));
...
whd_result_t whd_wifi_set_pmk(whd_interface_t ifp, const uint8_t *security_key, uint8_t key_length)
{
    ...
    whd_mem_memset(pmk, 0, sizeof(wsec_pmk_t) );

    if (key_length == WSEC_PMK_WPA3_ENT_192_LEN)
    {
        whd_mem_memcpy(pmk->key, security_key, key_length);
...
whd_result_t whd_wifi_set_pmksa(whd_interface_t ifp, const pmkid_t *pmkid)
{
    ...
    get_pmkid_list = (pmkid_list_t *)whd_proto_get_iovar_buffer(whd_driver, &buffer,
                                                  sizeof(uint32_t) + MAXPMKID *
                                                  sizeof(pmkid_t), IOVAR_STR_PMKID_INFO);
    CHECK_IOCTL_BUFFER(get_pmkid_list);
    get_pmkid_list->npmkid = get_pmkid_list->npmkid & 0x0000FFFF; // inform the h1combo we uses the v1 pmkid_list_t
    ...
    if (cnt == orig_pmkid_list->npmkid)
    {
        ...
        new_pmkid_list->npmkid = new_pmkid_list->npmkid & 0x0000FFFF; //inform the h1combo we uses the v1 pmkid_list_t
        whd_mem_memcpy(new_pmkid_list->pmkid, orig_pmkid_list->pmkid, orig_pmkid_list->npmkid * sizeof(pmkid_t) );
        CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
        whd_mem_memcpy(&new_pmkid_list->pmkid[new_pmkid_list->npmkid - 1], pmkid, sizeof(pmkid_t) );
    ...
    /* Replace Old PMKID for New PMKID under same BSSID and write the new multicast list */
    {
        ...
        whd_mem_memcpy(new_pmkid_list->pmkid, orig_pmkid_list->pmkid, orig_pmkid_list->npmkid * sizeof(pmkid_t) );
        CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
        whd_mem_memcpy(&new_pmkid_list->pmkid[cnt], pmkid, sizeof(pmkid_t) );
        new_pmkid_list->npmkid = htod32(new_pmkid_list->npmkid);
    }
...
whd_result_t whd_wifi_get_ap_client_rssi(whd_interface_t ifp, int32_t *rssi, const whd_mac_t *client_mac)
{
    ...
    whd_mem_memcpy(&client_rssi->macs, client_mac, sizeof(*client_mac) );
    client_rssi->rssi = 0;
    ...
    whd_mem_memcpy(rssi, data, sizeof(int32_t) );
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
                                          ...
                                          void *handler_user_data)
{
    ...
    switch (event_header->event_type)
    {
        ...
        case WLC_E_REASSOC:
             if (event_header->status == WLC_E_STATUS_SUCCESS)
             {
                 /* Setting necessary bits for connection success for the REASSOC case, as FW sends REASSOC event success
                    and then WLC_E_PSK_SUP */
                 whd_driver->internal_info.whd_join_status[event_header->bsscfgidx] |= (JOIN_LINK_READY | JOIN_SSID_SET);
             }
             break;
...
static whd_result_t whd_wifi_active_join_init(whd_interface_t ifp, whd_security_t auth_type, const uint8_t *security_key,
                                          uint8_t key_length, cy_semaphore_t *semaphore, whd_bool_t is_reassoc)
{
    whd_driver_t whd_driver = ifp->whd_driver;
...
whd_result_t whd_set_wsec_info_algos(whd_interface_t ifp, uint32_t algos, uint32_t mask)
{
        ...
        wsec_info->num_tlvs++;
        whd_mem_memcpy(tlv_data, &algos, sizeof(algos));
        whd_mem_memcpy(tlv_data + sizeof(algos), &mask, sizeof(mask));

        wsec_info_tlv->id = htod16(WL_WSEC_INFO_BSS_ALGOS);
        wsec_info_tlv->len = htod16(tlv_data_len);
        whd_mem_memcpy(wsec_info_tlv->data, tlv_data, tlv_data_len);
                                      ...
                                      cy_semaphore_t *semaphore)
{
    ...
    data[1] =
        htod32( ( uint32_t )( ( ( (auth_type & WPA_SECURITY) != 0 ) || ( (auth_type & WPA2_SECURITY) != 0 ) ||
                              ( (auth_type & WPA3_SECURITY) != 0 ) || ( (auth_type & WPA3_OWE) != 0 ) ) ? 1 : 0 ) );
    (void)whd_proto_set_iovar(ifp, buffer, 0);
    ...
    switch (auth_type)
    {
        ...
        case WHD_SECURITY_WPA3_OWE:
            /* Set the EAPOL key packet timeout value, otherwise unsuccessful supplicant events aren't reported. OWE needs 4-way handshake */
            CHECK_RETURN_UNSUPPORTED_CONTINUE(whd_wifi_set_supplicant_key_timeout(ifp,
                                                                                 DEFAULT_EAPOL_KEY_PACKET_TIMEOUT) );
            break;
    ...
    *wpa_auth = htod32(*wpa_auth);
    ...
    {
        CHECK_RETURN(whd_wifi_set_iovar_value(ifp, IOVAR_STR_SAE_TRANSITON_MODE, 0) );
    }
    else
    {
        CHECK_RETURN(whd_wifi_set_iovar_value(ifp, IOVAR_STR_SAE_TRANSITON_MODE, 1) );
    }
#endif /* CYW89530_AUTO */
...
static uint32_t whd_wifi_join_wait_for_complete(whd_interface_t ifp, cy_semaphore_t *semaphore)
{
    ...
    whd_driver_t whd_driver = ifp->whd_driver;
    uint32_t join_attempt_timeout;

    if (whd_driver->wifi_join_timeout) {
        join_attempt_timeout = whd_driver->wifi_join_timeout;
    } else {
        join_attempt_timeout = DEFAULT_JOIN_ATTEMPT_TIMEOUT;
    }
    ...
    while (!done)
    {
        ...
        switch (result)
        {
            ...
            case WHD_INVALID_JOIN_STATUS:
                ...
                break;
            default:
                done = (whd_bool_t)((current_time - start_time) >= join_attempt_timeout);
...
whd_result_t whd_set_join_option(whd_interface_t ifp, uint32_t option)
{
    ifp->join_option = option;
    return WHD_SUCCESS;
}
                                ...
                                uint8_t key_length)
{
    ...
    whd_bool_t is_reassoc = WHD_FALSE;
    CHECK_IFP_NULL(ifp);
    ...
    if ((whd_wifi_get_bss_info(ifp, &bss_info) == WHD_SUCCESS) &&
        (whd_wifi_is_ready_to_transceive(ifp) == WHD_SUCCESS))
    {
        if((strcmp((char *)ap->SSID.value, (char*)bss_info.SSID) == 0))
        {
            is_reassoc = WHD_TRUE;
        }
    }
    ...
    {
        WPRINT_WHD_DEBUG(("whd_wifi_get_bss_info failed going with WLC_SET_SSID \n"));
    }

    CHECK_RETURN(cy_rtos_init_semaphore(&join_semaphore, 1, 0) );
    result = whd_wifi_active_join_init(ifp, security, security_key, key_length, &join_semaphore, is_reassoc);
    ...
    if (result == WHD_SUCCESS)
    {
        ...
        if (!is_reassoc)
        {
            ...
            whd_mem_memset(ext_join_params, 0, sizeof(wl_extjoin_params_t) );

            ext_join_params->ssid.SSID_len = ap->SSID.length;
            DISABLE_COMPILER_WARNING(diag_suppress = Pa039)
            whd_mem_memcpy(ext_join_params->ssid.SSID, ap->SSID.value, ext_join_params->ssid.SSID_len);
            ENABLE_COMPILER_WARNING(diag_suppress = Pa039)
            whd_mem_memcpy(&ext_join_params->assoc_params.bssid, &ap->BSSID, sizeof(whd_mac_t) );
            ...
            if (result == WHD_WLAN_UNSUPPORTED)
            {
                ...
                whd_mem_memset(join_params, 0, sizeof(wl_join_params_t) );
                DISABLE_COMPILER_WARNING(diag_suppress = Pa039)
                whd_mem_memcpy(&join_params->ssid, &ext_join_params->ssid, sizeof(wlc_ssid_t) );
                ENABLE_COMPILER_WARNING(diag_suppress = Pa039)
                whd_mem_memcpy(&join_params->params.bssid, &ap->BSSID, sizeof(whd_mac_t) );
       ...
       {
            params = (wl_reassoc_params_t *)whd_proto_get_ioctl_buffer(whd_driver, &buffer,
                       (uint16_t)sizeof(wl_reassoc_params_t));
            whd_mem_memset(params, 0,  sizeof(wl_reassoc_params_t));
            whd_mem_memcpy(params->bssid.octet, ap->BSSID.octet, ETHER_ADDR_LEN);
            is_reassoc = WHD_FALSE;
            result = whd_proto_set_ioctl(ifp,
                       WLC_REASSOC, buffer, NULL);
        }
                       ...
                       const uint8_t *security_key, uint8_t key_length)
{
...
#ifdef PROTO_MSGBUF
        /* Make sure FW have enough RX data buffers before doing wifi join */
        if (whd_driver->msgbuf->rxbufpost <= (whd_driver->msgbuf->max_rxbufpost - WHD_MSGBUF_RXBUFPOST_THRESHOLD))
        {
            whd_msgbuf_rxbuf_data_fill(whd_driver->msgbuf);
        }
#endif /* PROTO_MSGBUF */

    CHECK_RETURN(cy_rtos_init_semaphore(&join_sema, 1, 0) );
    result = whd_wifi_active_join_init(ifp, auth_type, security_key, key_length, &join_sema, WHD_FALSE);
    ...
    if (result == WHD_SUCCESS)
    {
        ...
        whd_mem_memcpy(ssid_params->SSID, ssid->value, ssid_params->SSID_len);
        result = whd_proto_set_ioctl(ifp, WLC_SET_SSID, buffer, 0);
...
#ifdef PROTO_MSGBUF
    struct whd_flowring *flow = whd_driver->msgbuf->flow;
#endif
...
#ifdef PROTO_MSGBUF
    /* Clear the flowring created as part of STA */
    whd_flowring_delete_peers(flow, NULL, ifp->ifidx);
#endif
...

{
    ...
    supported_operating_classes_ie_t *supported_operating_classes_ie;
#endif
    ...
    whd_mem_memset(record, 0, sizeof(whd_scan_result_t) );
    ...
    whd_mem_memset(record->SSID.value, 0, sizeof(record->SSID.value) );
    whd_mem_memcpy(record->SSID.value, bss_info->SSID, record->SSID.length);

    /* Copy the BSSID into the output record structure */
    whd_mem_memcpy( (void *)record->BSSID.octet, (const void *)bss_info->BSSID.octet, sizeof(bss_info->BSSID.octet) );
    ...
    /* Copy the SNR into the output record structure */
    record->snr = ( int16_t )dtoh16((WHD_READ_16(&bss_info->SNR)));
    ...
    record->beacon_period = bss_info->beacon_period;
    record->capability = bss_info->capability;
         ...
         (rsnie->tlv_header.length >= RSN_IE_MINIMUM_LENGTH + rsnie->pairwise_suite_count * sizeof(uint32_t) ) )
    {
        ...
        for (a = 0; a < akm_suites->akm_suite_count; ++a)
        {
            ...
            if (akm_suite_list_item == (uint32_t)WHD_AKM_FT_SAE_SHA256)
            {
                record->security |= WPA3_SECURITY;
                record->fbtbitmap |= SAE_FBT;
            }
            ...
            if (akm_suite_list_item == (uint32_t)WHD_AKM_8021X_SHA256)
            {
                record->security |= ENTERPRISE_ENABLED;
                record->security |= WPA3_SECURITY;
                record->security |= SHA256_1X;
            }
            if (akm_suite_list_item == (uint32_t)WHD_AKM_8021X_SUITE_B_192)
            {
                record->security |= ENTERPRISE_ENABLED;
                record->security |= WPA3_SECURITY;
                record->security |= SUITE_B_SHA384;
            }
	    if (akm_suite_list_item == (uint32_t)WHD_AKM_FT_SAE_SHA256)
            {
                record->security |= WPA3_SECURITY;
            ...
            {
                record->security |= WPA2_SECURITY;
                record->fbtbitmap |= ENTERP_FBT;
                record->security |= ENTERPRISE_ENABLED;
            }
            if (akm_suite_list_item == (uint32_t)WHD_AKM_FT_PSK)
            {
                record->security |= WPA2_SECURITY;
                record->fbtbitmap |= PSK_FBT;
            }
            if (akm_suite_list_item == (uint32_t)WHD_AKM_OWE)
            {
                record->security |= WPA3_OWE;
            }
        }
        ...
        if (group_key_suite == (uint32_t)WHD_CIPHER_GCMP_256)
        {
            record->security |= AES_ENABLED;
        }
        ...
        for (a = 0; a < rsnie->pairwise_suite_count; ++a)
        {
            ...
            if (pairwise_suite_list_item == (uint32_t)WHD_CIPHER_GCMP_256)
            {
                record->security |= AES_ENABLED;
            }
        }
...
#ifdef COMPONENT_WIFI6
    /* Find supported operating classes IE */
    supported_operating_classes_ie = (supported_operating_classes_ie_t *)whd_parse_dot11_tlvs(cp, len, DOT11_IE_ID_SUPPORTED_OPERATING_CLASSES);
    if ((supported_operating_classes_ie != NULL) && (supported_operating_classes_ie->tlv_header.length >= SUPPORTED_OPERATING_CLASSES_IE_MINIMUM_LENGTH))
    {
       record->num_supported_operating_classes = MIN_OF(supported_operating_classes_ie->tlv_header.length, sizeof(record->supported_operating_classes));
       memcpy(record->supported_operating_classes, supported_operating_classes_ie->operating_classes, record->num_supported_operating_classes);
       record->current_operating_class = supported_operating_classes_ie->current_operating_class;
    }
#endif
...
static void whd_scan_count_handler(whd_scan_result_t **result_ptr, void *user_data, whd_scan_status_t status)
{
    ...
    whd_mem_memset(*result_ptr, 0, sizeof(whd_scan_result_t) );
    return;
}
...
static void whd_scan_result_handler(whd_scan_result_t **result_ptr, void *user_data, whd_scan_status_t status)
{
    ...
    whd_mem_memset(record->SSID.value, 0, sizeof(record->SSID.value) );
    whd_mem_memcpy(record->SSID.value, current_result->SSID.value, record->SSID.length);

    /* Copy the BSSID into the output record structure */
    whd_mem_memcpy( (void *)record->BSSID.octet, (const void *)current_result->BSSID.octet,
                             ...
                             )
{
    ...
    whd_mem_memset(scan_result_ptr, 0, sizeof(whd_scan_result_t) );
                       ...
                       )
{
    ...
    whd_driver_t whd_driver = ifp->whd_driver;
    uint16_t event_entry = 0xFF;
    uint16_t channel_list_2G_5G_6G[] = {0x1001, 0x1002, 0x1003, 0x1004, 0x1005, 0x1006, 0x1007,
        0x1008, 0x1009, 0x100A, 0x100B, 0xD038, 0xd024, 0xd028, 0xd02c, 0xd030, 0xd034, 0xd038,
        0xd03c, 0xd040, 0xd064, 0xd068, 0xd06c, 0xd070, 0xd074, 0xd078, 0xd07c, 0xd080, 0xd084,
        0xd088, 0xd08c, 0xd090, 0xd095, 0xd099, 0xd09d, 0xd0a1, 0xd0a5, 0x9005, 0x9015, 0x9025,
        0x9035, 0x9045, 0x9055, 0x9065, 0x9075, 0x9085, 0x9095, 0x90A5, 0x90B5, 0x90C5, 0x90D5,
        0x90E5, 0x0000};
    ...
    /* If ssid is not NULL, do directed scan  */
    if (optional_ssid != NULL && whd_wifi_platform_supports_triband(ifp))
    {
        optional_channel_list = channel_list_2G_5G_6G;
    }
    ...
    whd_mem_memset(scan_params, 0, param_size);
    ...
    if (optional_ssid != NULL)
    {
        ...
        whd_mem_memcpy(scan_params->params.ssid.SSID, optional_ssid->value, scan_params->params.ssid.SSID_len);
    }

    /* Fill out the BSSID parameter if provided */
    if (optional_mac != NULL)
    {
        whd_mem_memcpy(scan_params->params.bssid.octet, optional_mac, sizeof(whd_mac_t) );
    }
    else
    {
        whd_mem_memset(scan_params->params.bssid.octet, 0xff, sizeof(whd_mac_t) );
    }
...
#else
        whd_mem_memset(&(scan_params->params.nprobes), -1, 16);
#endif
    }
...
whd_result_t whd_wifi_stop_scan(whd_interface_t ifp)
{
    ...
    CHECK_IOCTL_BUFFER(scan_params);
    /* Clear the scan parameters structure */
    whd_mem_memset(scan_params, 0, sizeof(wl_escan_params_t) );
...
whd_result_t  whd_wifi_deauth_sta(whd_interface_t ifp, whd_mac_t *mac, whd_dot11_reason_code_t reason)
{
    ...
    if (mac == NULL)
    {
        ...
        whd_mem_memset(&ap_info, 0, sizeof(wl_bss_info_t) );
        ...
        while ( (clients->count > 0) && (!(NULL_MAC(current->octet) ) ) )
        {
            if (memcmp(current->octet, &(ap_info.BSSID), sizeof(whd_mac_t) ) != 0)
            {
                ...
                whd_mem_memset( (char *)scb_val, 0, sizeof(scb_val_t) );
                whd_mem_memcpy( (char *)&scb_val->ea, (char *)current, sizeof(whd_mac_t) );
    ...
    whd_mem_memset( (char *)scb_val, 0, sizeof(scb_val_t) );
    whd_mem_memcpy( (char *)&scb_val->ea, (char *)mac, sizeof(whd_mac_t) );
    scb_val->val = (uint32_t)reason;
...
whd_result_t whd_wifi_get_mac_address(whd_interface_t ifp, whd_mac_t *mac)
{
    ...
    whd_mem_memcpy(mac, whd_buffer_get_current_piece_data_pointer(whd_driver, response), sizeof(whd_mac_t) );
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
...
whd_result_t whd_wifi_get_bssid(whd_interface_t ifp, whd_mac_t *bssid)
{
    ...
    {
        whd_mem_memset(bssid, 0, sizeof(whd_mac_t) );
                  ...
                  whd_proto_get_ioctl(ifp, WLC_GET_BSSID, buffer, &response) ) == WHD_SUCCESS )
        {
            data = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
            CHECK_PACKET_NULL(data, WHD_NO_REGISTER_FUNCTION_POINTER);
            whd_mem_memcpy(bssid->octet, data, sizeof(whd_mac_t) );
            CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
        }
        return result;
    }
...
whd_result_t whd_wifi_ap_set_max_assoc(whd_interface_t ifp, uint32_t max_assoc)
{
    if (!ifp || !max_assoc)
    {
        WPRINT_WHD_ERROR( ("Invalid param in func %s at line %d \n",
    ...
    return whd_wifi_set_iovar_value(ifp, IOVAR_STR_MAX_ASSOC, max_assoc);
}
...
whd_result_t whd_wifi_get_associated_client_list(whd_interface_t ifp, void *client_list_buffer, uint16_t buffer_length)
{
    ...
    if ( (result == WHD_SUCCESS) && (ifp->role == WHD_AP_ROLE) )
    {
        ...
        whd_mem_memcpy(client_list_buffer, (void *)pdata,
...
whd_result_t whd_wifi_get_ap_info(whd_interface_t ifp, wl_bss_info_t *ap_info, whd_security_t *security)
{
    ...
    *data = WLC_IOCTL_SMLEN;
    ...
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy(ap_info, (void *)(pdata + 4), sizeof(wl_bss_info_t) );
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
    ...
    *security = ( whd_security_t )(security_value);
    ...
    else if ( (*security & (TKIP_ENABLED | AES_ENABLED) ) != 0 )
    {
        ...
        else if (security_value == WPA3_AUTH_SAE_PSK)
        {
            *security |= WPA3_SECURITY;
        }
        else if (security_value == WPA3_AUTH_OWE)
        {
            *security |= WPA3_OWE;
        }
    }
...
{
    whd_driver_t whd_driver;
    uint32_t get_pm_mode = 0, get_ulp_mode = 0;
    ...
    if( (wlan_chip_id == 43022) || (wlan_chip_id == 43012) )
    {
        WPRINT_WHD_DEBUG(("Connected Chip supports ULP \n"));
        whd_wifi_get_powersave_mode(ifp, &get_pm_mode);

        if((*mode == ULP_DS1_SUPPORT) || (*mode == ULP_DS2_SUPPORT))
        {
            if(get_pm_mode == NO_POWERSAVE_MODE)
            {
                WPRINT_WHD_DEBUG(("Power Save Mode Enabled - PM2 \n"));
                CHECK_RETURN(whd_wifi_set_iovar_value(ifp, IOVAR_STR_MPC, 0));
            ...
            if(get_ulp_mode == 0)
            {
                /* Enable wowl magic pattern wake bit and activate wowl module */
                CHECK_RETURN(whd_configure_wowl(ifp, WL_WOWL_MAGIC));
                CHECK_RETURN(whd_wifi_set_iovar_value(ifp, IOVAR_STR_WOWL_ACTIVATE, WHD_TRUE));
                /* Set ulp mode */
...
whd_result_t whd_wifi_register_multicast_address(whd_interface_t ifp, const whd_mac_t *mac)
{
    ...
    whd_mem_memcpy(new_mcast_list->macs, orig_mcast_list->macs, orig_mcast_list->entry_count * sizeof(whd_mac_t) );
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
    whd_mem_memcpy(&new_mcast_list->macs[new_mcast_list->entry_count], mac, sizeof(whd_mac_t) );
    ...
    if (orig_mcast_list->entry_count != 0)
    {
        ...
        for (a = 0; a < orig_mcast_list->entry_count; ++a)
        {
            ...
            if (0 == memcmp(mac, &orig_mcast_list->macs[a], sizeof(whd_mac_t) ) )
            {
                ...
                whd_mem_memcpy(new_mcast_list->macs, orig_mcast_list->macs, a * sizeof(whd_mac_t) );

                /* Skip the current address and copy the remaining entries */
                whd_mem_memcpy(&new_mcast_list->macs[a], &orig_mcast_list->macs[a + 1],
...
whd_result_t whd_wifi_get_listen_interval(whd_interface_t ifp, whd_listen_interval_t *li)
{
    ...
    CHECK_IOCTL_BUFFER(data);
    whd_mem_memset(data, 0, 1);
    CHECK_RETURN(whd_proto_get_iovar(ifp, buffer, &response) );
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy( (uint8_t *)&(li->beacon), (char *)pdata, 1 );
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
    ...
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy( (uint8_t *)&(li->dtim), (char *)pdata, 1 );
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );

    data = (int *)whd_proto_get_iovar_buffer(whd_driver, &buffer, 4, IOVAR_STR_LISTEN_INTERVAL_ASSOC);
    CHECK_IOCTL_BUFFER(data);
    whd_mem_memset(data, 0, 4);
    CHECK_RETURN(whd_proto_get_iovar(ifp, buffer, &response) );
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy( (uint16_t *)&(li->assoc), (char *)pdata, 2 );
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
...
whd_result_t whd_wifi_get_acparams(whd_interface_t ifp, edcf_acparam_t *acp)
{
    ...
    CHECK_IOCTL_BUFFER(data);
    whd_mem_memset(data, 0, 64);
    CHECK_RETURN(whd_proto_get_iovar(ifp, buffer, &response) );
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy( (char *)acp, (char *)pdata, (sizeof(edcf_acparam_t) * 4) );
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
...
whd_result_t whd_wifi_get_channels(whd_interface_t ifp, whd_list_t *channel_list)
{
    ...
    whd_mem_memset(list, 0, buffer_length);
    ...
    whd_mem_memcpy(channel_list, list,
                                   ...
                                   uint8_t subtype, const void *data, uint16_t length, uint16_t which_packets)
{
    ...
    *iovar_data = ifp->bsscfgidx;
    ...
    {
        whd_mem_memcpy( (char *)ie_setbuf->cmd, "add", 3 );
    }
    else
    {
        whd_mem_memcpy( (char *)ie_setbuf->cmd, "del", 3 );
    }
    ie_setbuf->cmd[3] = 0;
    ...
    whd_mem_memcpy(ie_setbuf->vndr_ie_buffer.vndr_ie_list[0].vndr_ie_data.oui, oui, (size_t)WIFI_IE_OUI_LENGTH);

    ie_setbuf->vndr_ie_buffer.vndr_ie_list[0].vndr_ie_data.data[0] = subtype;

    whd_mem_memcpy(&ie_setbuf->vndr_ie_buffer.vndr_ie_list[0].vndr_ie_data.data[1], data, length);
...
whd_result_t whd_wifi_send_action_frame(whd_interface_t ifp, whd_af_params_t *af_params)
{
    ...
    whd_mem_memcpy(af_frame, af_params, WL_WIFI_AF_PARAMS_SIZE);
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}
...
whd_result_t whd_wifi_send_auth_frame(whd_interface_t ifp, whd_auth_params_t *auth_params)
{
    ...
    whd_mem_memcpy(auth_frame, auth_params, OFFSET(whd_auth_params_t, data) );
    whd_mem_memcpy(auth_frame->data, &auth_params->data[DOT11_MGMT_HDR_LEN], auth_params->len);
...
whd_result_t whd_wifi_he_omi(whd_interface_t ifp, whd_he_omi_params_t *he_omi_params)
{
    ...
    whd_mem_memset( (uint8_t *)&he_omi_iovar, 0x00, sizeof(he_omi_iovar) );
    ...
    whd_mem_memcpy(he_omi_iovar->data, (uint8_t *)&he_omi, sizeof(wl_he_omi_t) );
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}
...
static void *whd_wifi_itwt_events_handler(whd_interface_t ifp, const whd_event_header_t *event_header,
                                          const uint8_t *event_data, void *handler_user_data)
{
    whd_driver_t whd_driver = ifp->whd_driver;
    const wl_twt_setup_cplt_t *twt_setup_cplt = (const wl_twt_setup_cplt_t *) event_data;

    if (event_header->event_type == WLC_E_TWT_SETUP)
    {
        const wl_twt_sdesc_t *desc = &twt_setup_cplt->desc;
        if (desc->negotiation_type == WHD_TWT_PARAM_NEGO_TYPE_ITWT)
        {
          switch (desc->setup_cmd)
          {
            ...
            case TWT_SETUP_CMD_GROUPING_TWT:
            case TWT_SETUP_CMD_ALTERNATE_TWT:
                WPRINT_WHD_ERROR(("TWT: Setup EVENT: Failed to update state %u \n", desc->setup_cmd));
                ifp->twt_negotiated_info.session_state = WHD_TWT_SESSION_STATE_SETUP_IN_PROGRESS;
                break;
            case TWT_SETUP_CMD_ACCEPT_TWT:
                ifp->twt_negotiated_info.session_state = WHD_TWT_SESSION_STATE_SETUP_COMPLETE;
                ifp->twt_negotiated_info.flow_id       = desc->flow_id;
                ifp->twt_negotiated_info.dialog_token  = twt_setup_cplt->dialog;
                ...
                cy_rtos_set_semaphore(&ifp->twt_event_semaphore, WHD_FALSE);
                break;
            case TWT_SETUP_CMD_REJECT_TWT:
            default:
                WPRINT_WHD_ERROR(("TWT Setup NOT complete. AP responded with intermediate/unsupported command: %u.\n", desc->setup_cmd));
                ifp->twt_negotiated_info.session_state = WHD_TWT_SESSION_STATE_NOT_COMPLETED;
                break;
          }
        }
        else
        {
            WPRINT_WHD_ERROR(("Unsupported TWT Negotiation Type received: %u \n", desc->negotiation_type));
            ifp->twt_negotiated_info.session_state = WHD_TWT_SESSION_STATE_INACTIVE;
        }
    }
    else if (event_header->event_type == WLC_E_TWT_TEARDOWN)
    {
         const wl_twt_teardown_t *teardown_event = (const wl_twt_teardown_t *)event_data;
         if (ifp->twt_negotiated_info.flow_id == teardown_event->teardesc.flow_id)
         {
                 ifp->twt_negotiated_info.session_state = WHD_TWT_SESSION_STATE_INACTIVE;
                 WPRINT_WHD_INFO(("TWT session state for Flow ID %u has been set to INACTIVE.\n", teardown_event->teardesc.flow_id));
         }
         if (whd_driver->internal_info.twt_setup_cplt_callback != NULL)
         {
            whd_driver->internal_info.twt_setup_cplt_callback(WHD_TWT_EVENT_TEARDOWN_COMPLETE, (void *)event_data);
         }
         cy_rtos_set_semaphore(&ifp->twt_event_semaphore, WHD_FALSE);
    }
...
whd_result_t whd_wifi_itwt_init(whd_interface_t ifp, whd_itwt_event_callback_t callback, void *user_data)
{
    CHECK_IFP_NULL(ifp);
    whd_driver_t whd_driver = ifp->whd_driver;
    uint16_t event_entry = 0xFF;

    CHECK_RETURN(cy_rtos_init_semaphore(&ifp->twt_event_semaphore, 1, 1));

    if (ifp->event_reg_list[WHD_TWT_EVENT_ENTRY] != WHD_EVENT_NOT_REGISTERED)
    {
        whd_wifi_deregister_event_handler(ifp, ifp->event_reg_list[WHD_TWT_EVENT_ENTRY]);
        ifp->event_reg_list[WHD_TWT_EVENT_ENTRY] = WHD_EVENT_NOT_REGISTERED;
    }

    /* Register our internal handler to catch TWT events */
    CHECK_RETURN(whd_management_set_event_handler(ifp, twt_setup_events, whd_wifi_itwt_events_handler, user_data, &event_entry));

    if (event_entry >= WHD_MAX_EVENT_SUBSCRIPTION)
    {
        WPRINT_WHD_ERROR(("TWT event registration failed in %s line %d\n", __func__, __LINE__));
        whd_driver->internal_info.twt_setup_cplt_callback = NULL;
        return WHD_UNFINISHED;
    }

    ifp->event_reg_list[WHD_TWT_EVENT_ENTRY] = event_entry;
    whd_driver->internal_info.twt_setup_cplt_callback = callback;

    return WHD_SUCCESS;

}

whd_result_t whd_wifi_deinit_twt(whd_interface_t ifp)
{
    whd_driver_t whd_driver;
    whd_result_t result = WHD_SUCCESS;
    ...
    CHECK_RETURN(cy_rtos_deinit_semaphore(&ifp->twt_event_semaphore));

    if (ifp->event_reg_list[WHD_TWT_EVENT_ENTRY] != WHD_EVENT_NOT_REGISTERED)
    {
        result = whd_wifi_deregister_event_handler(ifp, ifp->event_reg_list[WHD_TWT_EVENT_ENTRY]);
        if (result != WHD_SUCCESS)
        {
            WPRINT_WHD_ERROR(("Failed to deregister TWT event handler!\n"));
        }
        ifp->event_reg_list[WHD_TWT_EVENT_ENTRY] = WHD_EVENT_NOT_REGISTERED;
    }
    ...
    memset(&ifp->twt_negotiated_info, 0, sizeof(ifp->twt_negotiated_info));
    ifp->twt_negotiated_info.session_state = WHD_TWT_SESSION_STATE_INACTIVE;

    if (result != WHD_SUCCESS)
    {
        WPRINT_WHD_ERROR(("Failed to de-initialize TWT mutex!\n"));
    }
...
whd_result_t whd_wifi_itwt_setup(whd_interface_t ifp, whd_itwt_setup_params_t *twt_params)
{
    ...
    whd_mem_memset( (uint8_t *)&itwt_setup, 0x00, sizeof(itwt_setup) );
...
whd_result_t whd_wifi_btwt_join(whd_interface_t ifp, whd_btwt_join_params_t *twt_params)
{
    ...
    whd_mem_memset( (uint8_t *)&btwt_setup, 0x00, sizeof(btwt_setup) );
    ...
    twt_iovar->id = WL_TWT_CMD_SETUP;
    twt_iovar->len = sizeof(wl_twt_setup_t);
    whd_mem_memcpy(twt_iovar->data, (uint8_t *)&btwt_setup, sizeof(wl_twt_setup_t) );
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}
...
whd_result_t whd_wifi_twt_teardown(whd_interface_t ifp, whd_twt_teardown_params_t *twt_params)
{
    ...
    whd_mem_memset( (uint8_t *)&twt_teardown, 0x00, sizeof(twt_teardown) );
...
whd_result_t whd_wifi_twt_information_frame(whd_interface_t ifp, whd_twt_information_params_t *twt_params)
{
    ...
    whd_mem_memset( (uint8_t *)&twt_information, 0x00, sizeof(twt_information) );
    twt_information.version = WL_TWT_INFO_VER;
    twt_information.length = sizeof(wl_twt_info_t) - 4;
#ifdef PROTO_MSGBUF
    twt_information.infodesc.flow_flags = 0;
#else
    ...
    else
    {
        ...
        twt_information.infodesc.next_twt_h = 0;
        twt_information.infodesc.next_twt_l = (twt_params->resume_time * 1000000);
    }
    ...
    whd_mem_memcpy(twt_iovar->data, (uint8_t *)&twt_information, sizeof(wl_twt_info_t) );
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}

whd_result_t whd_wifi_itwt_get_negotiated_info(whd_interface_t ifp, whd_itwt_negotiated_params_t *negotiated_params)
{
    const whd_itwt_negotiated_params_t *negotiated_info = &ifp->twt_negotiated_info;
    whd_result_t result;

    if (negotiated_info->session_state != WHD_TWT_SESSION_STATE_SETUP_COMPLETE)
    {
        WPRINT_WHD_INFO(("No active TWT session information \n"));
        result = negotiated_info->session_state;
        return result;
    }

    whd_mem_memcpy(negotiated_params, negotiated_info, sizeof(whd_itwt_negotiated_params_t));
    WPRINT_WHD_DEBUG(("\n-------------------- TWT Session Info --------------------\n"));
    WPRINT_WHD_DEBUG((" iTWT Session, Flow ID: %u\n", negotiated_info->flow_id));
    WPRINT_WHD_DEBUG(("                 Session state       : Setup complete\n"));
    WPRINT_WHD_DEBUG(("                 Target Wake Time    : %lu uS\n", (unsigned long)negotiated_info->target_wake_time));
    WPRINT_WHD_DEBUG(("                 Wake Duration       : %lu uS\n", (unsigned long)negotiated_info->wake_duration));
    WPRINT_WHD_DEBUG(("                 Wake Interval       : %lu uS\n", (unsigned long)negotiated_info->wake_interval));
    WPRINT_WHD_DEBUG(("                 Dialog_token        : %u\n", negotiated_info->dialog_token));
    WPRINT_WHD_DEBUG(("                 Session type        : %s, %s, %s\n",
                     negotiated_info->is_implicit ? "Implicit" : "Explicit",
                     negotiated_info->is_triggered ? "Trigger based" : "Non-Trigger based",
                     negotiated_info->is_announced ? "Announced" : "Un-Announced"));
    WPRINT_WHD_DEBUG(("--------------------------------------------------------\n\n"));
...
whd_result_t whd_wifi_btwt_config(whd_interface_t ifp, whd_btwt_config_params_t *twt_params)
{
    ...
    whd_mem_memset( (uint8_t *)&config_btwt, 0x00, sizeof(config_btwt) );
    ...
    twt_iovar->id = WL_TWT_CMD_SETUP;
    twt_iovar->len = sizeof(wl_twt_setup_t);
    whd_mem_memcpy(twt_iovar->data, (uint8_t *)&config_btwt, sizeof(wl_twt_setup_t) );
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}
whd_result_t whd_wifi_get_he_color(whd_interface_t ifp, uint8_t *color)
{
    whd_buffer_t buffer;
    whd_buffer_t response;
    whd_driver_t whd_driver;
    whd_xtlv_t *color_iovar = NULL;
    uint8_t *pdata = NULL;
    uint16_t len;
    whd_result_t  ret;
    ...
    whd_driver = ifp->whd_driver;
    CHECK_DRIVER_NULL(whd_driver);
    len = sizeof(whd_xtlv_t)+sizeof(uint8_t)+1;
    color_iovar = (whd_xtlv_t *)whd_proto_get_iovar_buffer(whd_driver, &buffer, len,
                                                            IOVAR_STR_HE);
    CHECK_IOCTL_BUFFER(color_iovar);
    color_iovar->id = WL_HE_CMD_BSSCOLOR;
    color_iovar->len = sizeof(uint8_t);
    ...
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy(color,pdata, sizeof(uint8_t));
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
    return WHD_SUCCESS;
}
whd_result_t whd_wifi_set_he_color(whd_interface_t ifp, uint8_t *value)
{
    whd_buffer_t buffer;
    whd_xtlv_t  *color_iovar = NULL;
    whd_driver_t whd_driver;
    ...
    whd_driver = ifp->whd_driver;
    CHECK_DRIVER_NULL(whd_driver);
    whd_mem_memset( (uint8_t *)&color_iovar, 0x00, sizeof(color_iovar) );
    len = sizeof(whd_xtlv_t)+sizeof(uint8_t)+1;
    color_iovar = (whd_xtlv_t *)whd_proto_get_iovar_buffer(whd_driver, &buffer, len,
                                                            IOVAR_STR_HE);
    CHECK_IOCTL_BUFFER (color_iovar);
    color_iovar->id = WL_HE_CMD_BSSCOLOR;
    color_iovar->len = sizeof(uint8_t);
    whd_mem_memcpy(color_iovar->data, value , sizeof(uint8_t) );
    CHECK_RETURN(whd_proto_set_iovar(ifp, buffer, NULL) );
    return WHD_SUCCESS;
}
whd_result_t whd_wifi_oce_enable(whd_interface_t ifp, whd_bool_t value)
{
    whd_buffer_t buffer;
    whd_iov_buf_t *oce_iovar;
    whd_driver_t whd_driver;
    mbo_xtlv_t param;
    ...
    whd_mem_memset( (uint8_t *)&param, 0x00, sizeof(param) );
    param.id = WL_OCE_XTLV_ENABLE;
    param.len = sizeof(value);
    whd_mem_memcpy(param.data, (uint32_t *)&value, sizeof(value));

    oce_iovar = (whd_iov_buf_t *)whd_proto_get_iovar_buffer(whd_driver, &buffer, sizeof(whd_iov_buf_t) + sizeof(param),
                                                         IOVAR_STR_OCE);
    CHECK_IOCTL_BUFFER (oce_iovar);
    oce_iovar->version = WL_OCE_IOV_VERSION;
    oce_iovar->len = sizeof(param);
    oce_iovar->id = WL_OCE_CMD_ENABLE;
    whd_mem_memcpy(oce_iovar->data, (uint16_t *)&param, sizeof(mbo_xtlv_t) );
    CHECK_RETURN(whd_proto_set_iovar(ifp, buffer, NULL));
    return WHD_SUCCESS;
}
whd_result_t whd_wifi_mbo_add_chan_pref(whd_interface_t ifp, whd_mbo_add_chan_pref_params_t *mbo_params)
{
    whd_buffer_t buffer;
    ...
    whd_mem_memset( (uint8_t *)&ch_pref, 0x00, sizeof(ch_pref) );
    //opclass
    ch_pref.opclass.id = WL_MBO_XTLV_OPCLASS;
    ch_pref.opclass.len = sizeof(mbo_params->opclass);
    whd_mem_memcpy(ch_pref.opclass.data, (uint32_t *)&mbo_params->opclass, sizeof(mbo_params->opclass));
    //channel
    ch_pref.chan.id = WL_MBO_XTLV_CHAN;
    ch_pref.chan.len = sizeof(mbo_params->chan);
    whd_mem_memcpy(ch_pref.chan.data, (uint32_t *)&mbo_params->chan, sizeof(mbo_params->chan));
    //channel preference
    ch_pref.pref.id = WL_MBO_XTLV_PREFERENCE;
    ch_pref.pref.len = sizeof(mbo_params->pref);
    whd_mem_memcpy(ch_pref.pref.data, (uint32_t *)&mbo_params->pref, sizeof(mbo_params->pref));
    //reason
    ch_pref.reason.id = WL_MBO_XTLV_REASON_CODE;
    ch_pref.reason.len = sizeof(mbo_params->reason);
    whd_mem_memcpy(ch_pref.reason.data, (uint32_t *)&mbo_params->reason, sizeof(mbo_params->reason));
    ...
    whd_mem_memcpy(mbo_iovar->data, (uint16_t *)&ch_pref, sizeof(mbo_add_chan_pref_t) );
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}

whd_result_t whd_wifi_mbo_del_chan_pref(whd_interface_t ifp, whd_mbo_del_chan_pref_params_t *mbo_params)
{
    whd_buffer_t buffer;
    whd_iov_buf_t *mbo_iovar;
    whd_driver_t whd_driver;
    ...
    whd_mem_memcpy(mbo_iovar->data, (uint16_t *)&ch_pref, sizeof(mbo_del_chan_pref_t) );
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}

whd_result_t whd_wifi_mbo_send_notif(whd_interface_t ifp, uint8_t sub_elem_type)
{
    whd_buffer_t buffer;
    whd_iov_buf_t *mbo_iovar;
    whd_driver_t whd_driver;
    ...
    whd_mem_memset( (uint8_t *)&sub_elem, 0x00, sizeof(sub_elem) );
    sub_elem.id = WL_MBO_XTLV_SUB_ELEM_TYPE;
    sub_elem.len = sizeof(sub_elem_type);
    whd_mem_memcpy(sub_elem.data, (uint32_t *)&sub_elem_type, sizeof(sub_elem_type));
    ...
    whd_mem_memcpy(mbo_iovar->data, (uint16_t *)&sub_elem, sizeof(mbo_xtlv_t) );
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}
...
whd_result_t whd_wifi_set_ioctl_buffer(whd_interface_t ifp, uint32_t ioctl, void *in_buffer, uint16_t in_buffer_length)
{
    ...
    whd_mem_memcpy(data, in_buffer, in_buffer_length);
...
whd_result_t whd_wifi_get_ioctl_buffer(whd_interface_t ifp, uint32_t ioctl, uint8_t *out_buffer, uint16_t out_length)
{
    ...
    if (WHD_SUCCESS == result)
    {
        ...
        CHECK_PACKET_NULL(data, WHD_NO_REGISTER_FUNCTION_POINTER);
        *data = dtoh32(*data);
        whd_mem_memcpy(out_buffer, data, out_length);
        CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
    }
                                                  ...
                                                  uint32_t paramlen, uint8_t *out_buffer, uint32_t out_length)
{
    ...
    if (result == WHD_SUCCESS)
    {
        whd_mem_memcpy(out_buffer, whd_buffer_get_current_piece_data_pointer(whd_driver, response),
                                    ...
                                    const uint16_t *lengths, const uint8_t num_buffers)
{
    ...
    {
        whd_mem_memcpy(data, in_buffers[buffer_num], lengths[buffer_num]);
...
whd_result_t whd_wifi_get_bss_info(whd_interface_t ifp, wl_bss_info_t *bi)
{
    ...
    *data = WLC_IOCTL_SMLEN;
    ...
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy(bi, pdata  + 4, sizeof(wl_bss_info_t) );
...
whd_result_t whd_wifi_set_auth_h2e_cap(whd_interface_t ifp, bool h2ecap)
{
    whd_result_t result = CY_RSLT_SUCCESS;


    result = whd_wifi_set_iovar_value(ifp, IOV_SAE_PWE_H2E_ENABLE, h2ecap);

    if (result == WHD_SUCCESS)
    {
        WPRINT_WHD_ERROR(("\nSuccesfully set H2E cap in FW"));
    }
    else
    {
...
whd_result_t whd_wifi_set_auth_status(whd_interface_t ifp, whd_auth_req_status_t *params)
{
    ...
    whd_mem_memcpy(auth_status, params, sizeof(whd_auth_req_status_t) );
...
whd_result_t whd_arp_hostip_list_clear_id(whd_interface_t ifp, uint32_t ipv4_addr)
{
    ...
    whd_mem_memset(host_ipv4_list, 0x00, sizeof(host_ipv4_list) );
...
whd_result_t whd_arp_hostip_list_get(whd_interface_t ifp, uint32_t count, uint32_t *host_ipv4_list, uint32_t *filled)
{
    ...
    whd_mem_memset(&arp_stats, 0x00, sizeof(arp_ol_stats_t) );
...
whd_result_t whd_arp_stats_get(whd_interface_t ifp, whd_arp_stats_t *arp_stats)
{
    ...
    whd_mem_memcpy(&arp_stats_test, arp_stats, sizeof(whd_arp_stats_t) );
    whd_mem_memset(arp_stats, 0xFF, sizeof(whd_arp_stats_t) );
...
whd_result_t whd_arp_stats_print(whd_arp_stats_t *arp_stats, const char *title)
{
    ...
    if (arp_stats->stats.host_ip_entries > 0)
    {
        ...
        for (index = 0; index < arp_stats->stats.host_ip_entries; index++)
        {
            ...
            char ipv4_string[32];
            whd_mem_memset(ipv4_string, 0x00, sizeof(ipv4_string) );
...
#else
    whd_mem_memcpy(&data->id, &filter_id, sizeof(filter_id));
    whd_mem_memcpy(&data->enable, &enable, sizeof(enable) );
#endif /* PROTO_MSGBUF */
    RETURN_WITH_ASSERT(whd_proto_set_iovar(ifp, buffer, NULL) );
}
...
whd_pf_add_packet_filter(whd_interface_t ifp, const whd_packet_filter_t *settings)
{
    ...
    /* Copy mask */
    whd_mem_memcpy(packet_filter->u.pattern.mask_and_pattern, settings->mask, settings->mask_size);

    /* Copy filter pattern */
    whd_mem_memcpy(packet_filter->u.pattern.mask_and_pattern + settings->mask_size, settings->pattern, settings->mask_size);
...
whd_pf_get_packet_filter_stats(whd_interface_t ifp, uint8_t filter_id, whd_pkt_filter_stats_t *stats)
{
    ...
    whd_mem_memset(data, 0, sizeof(uint32_t) + sizeof(wl_pkt_filter_stats_t) );
    *data = (uint32_t)filter_id;

    CHECK_RETURN(whd_proto_get_iovar(ifp, buffer, &response) );
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy( (char *)stats, (char *)pdata, (sizeof(wl_pkt_filter_stats_t) ) );
        ...
        for (i = 0; i < filter_list->num; i++)
        {
            ...
            if (in_filter->id == filter_id)
            {
                *size_out = MIN_OF(in_filter->u.pattern.size_bytes, max_size);
                whd_mem_memcpy (mask,    in_filter->u.pattern.mask_and_pattern, *size_out);
                whd_mem_memcpy (pattern, in_filter->u.pattern.mask_and_pattern + in_filter->u.pattern.size_bytes, *size_out);
                CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_RX) );
...
whd_tko_param(whd_interface_t ifp, whd_tko_retry_t *whd_retry, uint8_t set)
{
    ...
    else
    {
        ...
        if (result == WHD_SUCCESS)
        {
            ...
            whd_mem_memcpy( (char *)wl_param_p,
...
whd_tko_get_status(whd_interface_t ifp, whd_tko_status_t *whd_status)
{
    ...
    if (tko)
    {
        ...
        {
            whd_mem_memcpy(whd_status, tko->data, MAX_TKO_CONN + 1);
        }
    }
    CHECK_RETURN(whd_buffer_release(whd_driver, response, WHD_NETWORK_TX) );
    return result;
...
whd_tko_max_assoc(whd_interface_t ifp, uint8_t *max)
{
    ...
    pdata = whd_buffer_get_current_piece_data_pointer(whd_driver, response);
    CHECK_PACKET_NULL(pdata, WHD_NO_REGISTER_FUNCTION_POINTER);
    whd_mem_memcpy( (char *)tko_max_tcp,
...
whd_tko_get_FW_connect(whd_interface_t ifp, uint8_t index, whd_tko_connect_t *whd_connect, uint16_t buflen)
{
    ...
    if (tko->len >= sizeof(*connect) )
    {
        ...
        if (connect->ip_addr_type == 0)
        {
             ...
             */
            whd_mem_memcpy(whd_connect, connect, MIN_OF(mylen, buflen) );
        }
    }
    return WHD_SUCCESS;
}
...
whd_tko_filter(whd_interface_t ifp, whd_tko_auto_filter_t * whd_filter, uint8_t filter_flag)
{
   ...
   CHECK_IOCTL_BUFFER(data);
	...
	if(filter_flag & TKO_FILTER_SRC_IP)
		whd_mem_memcpy(wl_filter->ip_src,whd_filter->ip_src,IPV6_ADDR_LEN);

	if(filter_flag & TKO_FILTER_DST_IP)
		whd_mem_memcpy(wl_filter->ip_dst,whd_filter->ip_dst,IPV6_ADDR_LEN);
...
whd_result_t whd_wifi_get_fw_logs(whd_interface_t ifp)
{
    whd_driver_t whd_driver = ifp->whd_driver;
    return whd_wifi_print_whd_log(whd_driver);
}

whd_result_t whd_wifi_get_cr4_tcm_byte(whd_interface_t ifp, uint32_t offset)
{
    return whd_wifi_read_tcm_byte(ifp->whd_driver, offset);
}

whd_result_t whd_wifi_enable_event_log(whd_interface_t ifp, uint8_t category)
{
    uint32_t result;
    wl_el_tag_params_s_t el_config[] = {
        /* scan */
        {28, 1, EVTLOG_LOG_AND_PRINT_MODE}, {29, 1, EVTLOG_LOG_AND_PRINT_MODE}, {30, 1, EVTLOG_LOG_AND_PRINT_MODE},
        {31, 1, EVTLOG_LOG_AND_PRINT_MODE}, {113,1, EVTLOG_LOG_AND_PRINT_MODE}, {128, 1, EVTLOG_LOG_AND_PRINT_MODE},
        /* mfp */
        {32, 1, EVTLOG_LOG_AND_PRINT_MODE}, {33, 1, EVTLOG_LOG_AND_PRINT_MODE}, {34, 1, EVTLOG_LOG_AND_PRINT_MODE},
        {35, 1, EVTLOG_LOG_AND_PRINT_MODE},
        /* assoc */
        {73, 1, EVTLOG_LOG_AND_PRINT_MODE}, {127, 1, EVTLOG_LOG_AND_PRINT_MODE},
        /* wsec */
        {77, 1, EVTLOG_LOG_AND_PRINT_MODE}, {78, 1, EVTLOG_LOG_AND_PRINT_MODE},
        /* key_mgmt */
        {131, 1, EVTLOG_LOG_AND_PRINT_MODE}, {136, 1, EVTLOG_LOG_AND_PRINT_MODE}, {137, 1, EVTLOG_LOG_AND_PRINT_MODE},
        /* 4-way handshake */
        {143, 1, EVTLOG_LOG_AND_PRINT_MODE},
        /* sae */
        {287, 1, EVTLOG_LOG_AND_PRINT_MODE}, {288, 1, EVTLOG_LOG_AND_PRINT_MODE},
        /* owe */
        {312, 1, EVTLOG_LOG_AND_PRINT_MODE}, {313, 1, EVTLOG_LOG_AND_PRINT_MODE},
    };

    start_ptr_and_iter_t categories[] = {{0,6}, {6,4}, {10,2}, {12,2}, {14,3}, {17,1}, {18,2}, {20,2},};

    for (uint8_t enable_idx = categories[category].start_idx;
                              enable_idx < categories[category].iterations;
                              enable_idx++)
    {
        result = whd_wifi_set_iovar_buffer(ifp,
                                           IOVAR_STR_EVENT_LOG,
                                           (void *)(&el_config[enable_idx]),
                                           sizeof(wl_el_tag_params_s_t));
        if (result != WHD_SUCCESS)
        {
            WPRINT_WHD_ERROR( ("whd_wifi_enable_event_log: IOVAR for el tag %d FAILED with error %ld\n",
                                            el_config[enable_idx].tag, (unsigned long)result) );
        }
    }
...
whd_result_t whd_wifi_set_mac_addr_via_otp(whd_interface_t ifp, char *tlvBuf, uint8_t len)
{
    whd_buffer_t buffer;
    whd_driver_t whd_driver = ifp->whd_driver;
    char *data;
    ...
    data = (char *)whd_proto_get_iovar_buffer(whd_driver, &buffer, (uint16_t)len, IOVAR_STR_OTPTLV);
    CHECK_IOCTL_BUFFER(data);
    memcpy(data, tlvBuf, len);
    result = whd_proto_set_iovar(ifp, buffer, 0);
		     ...
		     uint8_t * mask, uint8_t pattern_size, uint8_t * pattern, uint8_t type)
{
...
#ifdef PROTO_MSGBUF
    wl_wowl_pattern_t wl_pattern_cpy;
#endif
    ...
    {
#ifdef PROTO_MSGBUF
        wl_pattern_cpy.offset = offset;
        wl_pattern_cpy.masksize = mask_size;
        wl_pattern_cpy.patternsize = pattern_size;
        wl_pattern_cpy.patternoffset = sizeof(wl_wowl_pattern_t) + mask_size;
        wl_pattern_cpy.id = 0;
        wl_pattern_cpy.reasonsize = 0x00;
        wl_pattern_cpy.type = type;
        memcpy(wl_pattern, &wl_pattern_cpy, sizeof(wl_wowl_pattern_t));
#else
...
whd_get_wowl_pattern(whd_interface_t ifp,uint32_t pattern_num,  wl_wowl_pattern_t * pattern)
{
    ...
    whd_mem_memcpy(pattern, list->pattern, tot);
...
whd_wowl_activate_secure(whd_interface_t ifp, tls_param_info_t *tlsparam)
{
    ...
    whd_mem_memcpy(data, tlsparam, sizeof(tls_param_info_t));
    ...
    {
        WPRINT_WHD_ERROR( ("whd_wowl_activate_secure failed and error - %lu\n", (unsigned long)ret));
    }
    return ret;
}
...
#ifdef PROTO_MSGBUF
void whd_wifi_delete_peer(whd_interface_t ifp, uint8_t *peer_addr)
{
    whd_driver_t whd_driver = ifp->whd_driver;
    struct whd_flowring *flow = whd_driver->msgbuf->flow;

    if(!(NULL_MAC(peer_addr)))
    {
        WPRINT_WHD_DEBUG(("Clear flowring for STA: %.2X:%.2X:%.2X:%.2X:%.2X:%.2X\n", peer_addr[0],
                            peer_addr[1], peer_addr[2], peer_addr[3], peer_addr[4], peer_addr[5]));
        whd_flowring_delete_peers(flow, peer_addr, ifp->ifidx);
    }
    return;
}
#endif
...
whd_wifi_get_deepsleep_stats(whd_driver_t whd_driver, char *buf, uint32_t buflen)
{
    ...
    ret = whd_wifi_get_iovar_buffer_with_param(ifp, IOVAR_STR_DUMP, param, (uint32_t)strlen(param), buf, buflen);
    ...
    {
        WPRINT_WHD_ERROR( ("Failed to get ULP statistics, error code: %lu", ret) );
    }
...
whd_configure_scanmac_randomisation(whd_interface_t ifp, whd_bool_t config)
{
    whd_result_t result;
    /* set */
    uint8_t buffer[offsetof(wl_scanmac_t, data) +
            sizeof(wl_scanmac_enable_t)];
                                                         ...
                                                         whd_icmp_echo_req_info_t *whd_peer_info)
{
    ...
    if ( !(ifp->whd_driver->chip_info.fwcap_flags & (1 << WHD_FWCAP_ICMP) ) )
    {
        return WHD_UNSUPPORTED;
    }
...
whd_result_t whd_wifi_get_country_code(whd_interface_t ifp, char* country_abbrev)
{
    wl_country_t cspec;
    ...
    CHECK_RETURN(whd_wifi_get_iovar_buffer(ifp, IOVAR_STR_COUNTRY, (uint8_t*)&cspec, sizeof(cspec)));

    memcpy(country_abbrev, cspec.country_abbrev, sizeof(cspec.country_abbrev));

    return WHD_SUCCESS;
}


whd_result_t whd_wifi_get_country_list(whd_interface_t ifp, uint32_t band, uint32_t* count, char* country_list)
{
    uint8_t buf[512];
    ...
    cl->buflen = WLC_IOCTL_SMLEN;
    cl->count = 0;

    if (band > 0)
    {
        /* Band information available */
        cl->band = band;
        cl->band_set = 1;
    }
    else
    ...
    CHECK_RETURN(whd_wifi_get_ioctl_buffer(ifp, WLC_GET_COUNTRY_LIST, buf, WLC_IOCTL_SMLEN));

    memcpy((void*)country_list, (void*)&cl->country_abbrev[0], WLC_CNTRY_BUF_SZ * dtoh32(cl->count));
    *count = dtoh32(cl->count);

    return WHD_SUCCESS;
}

whd_result_t whd_set_wifi_join_timeout(whd_interface_t ifp, uint32_t timeout)
{
    CHECK_IFP_NULL(ifp);
    CHECK_DRIVER_NULL(ifp->whd_driver);

    if (timeout < WIFI_JOIN_MIN_TIMEOUT || timeout > WIFI_JOIN_MAX_TIMEOUT)
    {
        WPRINT_WHD_ERROR( ("WiFi join timeout range value is %d - %d.", WIFI_JOIN_MIN_TIMEOUT, WIFI_JOIN_MAX_TIMEOUT) );
        return WHD_BADARG;
    }

    ifp->whd_driver->wifi_join_timeout = timeout;
